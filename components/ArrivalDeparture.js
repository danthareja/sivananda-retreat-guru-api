...
export default class ArrivalDeparture extends Component {
  getTitle() {
    ...
    return `${trim(program.name)}: Arrivals and Departures between ${
      query.min_stay
    } and ${query.max_stay}`;
  }
  ...
  getTaxiPickupTime(flightTime, direction) {
    ...
    if (direction === "Arrival") {
      ...
      throw new Error(
        'Invalid direction. Expected either "Arrival" or "Departure"'
      );
    }
      ...
      .map(date => {
        const arrivals = registrations
          ...
          .map(r => ({
            ...
            taxiPickup: this.getTaxiPickupTime(
              r.questions.flight_arrival_time_in_nassau_2,
              "Arrival"
            )
          }));
        ...
        const departures = registrations
          ...
          .map(r => ({
            ...
            flightNumber:
              r.questions.flight_airline_and_flight_number_for_nassau_departure,
            location: r.questions.arriving_by_boat_or_at_the_back_gate,
            taxiPickup: this.getTaxiPickupTime(
              r.questions.flight_departure_time_from_nassau,
              "Departure"
            )
          }));
